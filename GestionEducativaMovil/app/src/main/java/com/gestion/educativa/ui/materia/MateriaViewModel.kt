...
import com.gestion.educativa.data.model.MateriaRequest
...
data class MateriaUiState(
    val items: List<Materia> = emptyList(),
    val selected: Materia? = null,
...
class MateriaViewModel @Inject constructor(
    ...
    private val _state = MutableStateFlow(MateriaUiState())
    val state: StateFlow<MateriaUiState> = _state.asStateFlow()
    ...
    fun save(id: Int?, carreraId: Int, docenteId: Int, nombre: String, codigo: String,
             creditos: Int, semestre: Int, activo: Boolean, onSuccess: () -> Unit) {
        viewModelScope.launch {
            _state.update { it.copy(isLoading = true, error = null) }
            val req = MateriaRequest(carreraId, docenteId, nombre, codigo, creditos, semestre, activo)
            val result = if (id == null) repository.create(req) else repository.update(id, req)
            when (result) {
                is Resource.Success -> {
                    _state.update { it.copy(isLoading = false, successMessage = if (id == null) "Materia creada" else "Materia actualizada") }
                    onSuccess()
                }
            ...
            when (val r = repository.delete(id)) {
                is Resource.Success -> {
                    _state.update { it.copy(isLoading = false, items = _state.value.items.filter { it.id != id },
                        totalCount = _state.value.totalCount - 1, successMessage = "Materia eliminada") }
                    onSuccess()
                }
                is Resource.Error -> _state.update { it.copy(isLoading = false, error = r.message) }
                else -> {}
