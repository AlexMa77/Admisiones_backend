...
import com.gestion.educativa.data.model.MatriculaRequest
...
data class MatriculaUiState(
    val items: List<Matricula> = emptyList(),
    val selected: Matricula? = null,
...
class MatriculaViewModel @Inject constructor(
    ...
    private val _state = MutableStateFlow(MatriculaUiState())
    val state: StateFlow<MatriculaUiState> = _state.asStateFlow()
    var searchQuery = MutableStateFlow(""); private set
    var estadoFilter = MutableStateFlow(""); private set

    init { load(reset = true) }

    fun onSearch(q: String) { searchQuery.value = q; load(reset = true) }
    fun onEstadoFilter(e: String) { estadoFilter.value = e; load(reset = true) }
    ...
    fun load(reset: Boolean = false) {
        viewModelScope.launch {
            ...
            when (val r = repository.list(page, searchQuery.value, estadoFilter.value)) {
                is Resource.Success -> {
    ...
    fun save(id: Int?, estudianteId: Int, materiaId: Int, periodo: String,
             estado: String, fecha: String, onSuccess: () -> Unit) {
        viewModelScope.launch {
            _state.update { it.copy(isLoading = true, error = null) }
            val req = MatriculaRequest(estudianteId, materiaId, periodo, estado, fecha)
            val result = if (id == null) repository.create(req) else repository.update(id, req)
            when (result) {
                is Resource.Success -> {
                    _state.update { it.copy(isLoading = false, successMessage = if (id == null) "Matrícula creada" else "Matrícula actualizada") }
                    onSuccess()
                }
            ...
            when (val r = repository.delete(id)) {
                is Resource.Success -> {
                    _state.update { it.copy(isLoading = false, items = _state.value.items.filter { it.id != id },
                        totalCount = _state.value.totalCount - 1, successMessage = "Matrícula eliminada") }
                    onSuccess()
                }
                is Resource.Error -> _state.update { it.copy(isLoading = false, error = r.message) }
                else -> {}
