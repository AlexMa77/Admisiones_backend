...
import com.gestion.educativa.data.model.CarreraRequest
...
data class CarreraUiState(
    val items: List<Carrera> = emptyList(),
    val selected: Carrera? = null,
...
class CarreraViewModel @Inject constructor(
    ...
    private val _state = MutableStateFlow(CarreraUiState())
    val state: StateFlow<CarreraUiState> = _state.asStateFlow()
    ...
    fun save(id: Int?, facultad: Int, nombre: String, codigo: String,
             duracion: Int, activo: Boolean, onSuccess: () -> Unit) {
        viewModelScope.launch {
            _state.update { it.copy(isLoading = true, error = null) }
            val req = CarreraRequest(facultad, nombre, codigo, duracion, activo)
            val result = if (id == null) repository.create(req) else repository.update(id, req)
            when (result) {
                is Resource.Success -> {
                    _state.update { it.copy(isLoading = false,
                        successMessage = if (id == null) "Carrera creada" else "Carrera actualizada") }
                    onSuccess()
                }
            ...
            when (val r = repository.delete(id)) {
                is Resource.Success -> {
                    _state.update { it.copy(isLoading = false,
                        ...
                        totalCount = _state.value.totalCount - 1, successMessage = "Carrera eliminada") }
                    onSuccess()
                }
                is Resource.Error -> _state.update { it.copy(isLoading = false, error = r.message) }
                else -> {}
